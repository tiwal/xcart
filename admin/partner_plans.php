 ...
 * @version    $Id: partner_plans.php,v 1.40.2.8.2.1 2012/04/06 13:01:28 aim Exp $
    ...
    if ($planid) {
        ...
        if (is_array($partner_plan_info['commissions'])) {
            ...
            foreach($partner_plan_info['commissions'] as $k => $v) {
                ...
                switch ($v['item_type']) {
                    case 'P':
                        $partner_plan_info['commissions'][$k]["product"] = func_query_first_cell("SELECT product FROM $sql_tbl[products_lng_current] WHERE productid='$v[item_id]'");
                        break;
