 ...
 * @version    $Id: html_catalog.php,v 1.136.2.10.2.1 2012/04/06 13:01:27 aim Exp $
...
if (empty($site_location['port']))
    $site_location['port_str'] = '';
else    
    $site_location['port_str'] = ':' . $site_location['port'];
    ...
    if ($categories_data) {
        ...
        while ($category_data = db_fetch_array($categories_data)) {
            ...
            if (($hc_state['gen_action'] & 2) === 2) {
                ...
                $products_data = db_query("
                SELECT $sql_tbl[products].productid, $sql_tbl[products_lng_current].product
                  FROM $sql_tbl[products_categories]
                 INNER JOIN $sql_tbl[products]
                    ON $sql_tbl[products_categories].categoryid =  '$category_data[categoryid]'
                   AND $sql_tbl[products_categories].productid  =  $sql_tbl[products].productid
                   AND $sql_tbl[products].forsale               != 'N'
                 ...
                 ON $sql_tbl[products_lng_current].productid=$sql_tbl[products].productid
                  LEFT JOIN $sql_tbl[product_memberships]
                    ON $sql_tbl[products].productid             =  $sql_tbl[product_memberships].productid $avail_join
                 WHERE $sql_tbl[product_memberships].productid IS NULL $prod_cond $avail_condition $avail_group_by
                 ORDER BY $sql_tbl[products].productid");
