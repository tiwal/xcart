 ...
 * @version    $Id: atracking.php,v 1.48.2.8 2012/03/27 11:18:15 aim Exp $
 ...
 */
    ...
    $all_views = func_query_first_cell("SELECT COUNT(date) FROM $sql_tbl[stats_pages_paths] WHERE $date_condition");
    $statistics = func_query("SELECT path, COUNT(path) as counter FROM $sql_tbl[stats_pages_paths] WHERE $date_condition GROUP BY path ORDER BY counter DESC LIMIT 25");
    if (!empty($statistics))
        foreach($statistics as $k => $v) {
