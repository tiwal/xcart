 ...
 * @version    $Id: languages.php,v 1.111.2.14.2.2 2012/04/06 13:01:28 aim Exp $
...
if (!empty($language) && !empty($mode)) {
    ...
    if ($mode == 'update_charset') {
        ...
        func_drop_lng_tables('feature_variants_lng_', $language);
        func_drop_lng_tables('products_lng_', $language);
        ...
        if (!empty($active_modules['Flyout_Menus'])) {
            func_fc_remove_cache(10, false, array($language));
        }
...
if ($mode == 'add_lang') {
    ...
    func_add_lng_table('feature_variants_lng_', $new_language);
    func_add_lng_table('products_lng_', $new_language);

    $all_languages = func_data_cache_get('languages', array($shop_language), true);
    $sql_tbl['feature_variants_lng_' . $new_language] = $xcart_tbl_prefix . 'feature_variants_lng_' . $new_language;
    $sql_tbl['products_lng_' . $new_language] = $xcart_tbl_prefix . 'products_lng_' . $new_language;

    func_repair_lng_integrity('feature_variants_lng_', $sql_tbl['feature_variants'], 'fvariantid', "'restored_entity' AS variant_name");
