 ...
 * @version    $Id: ps_paypal.php,v 1.59.2.9.2.1 2012/04/18 08:40:24 aim Exp $
...
if ((isset($_GET['mode']) && $_GET['mode'] == 'success') || (isset($_POST['mode']) && $_POST['mode'] == 'success')) {
    ...
    if (defined('PAYPAL_STANDARD_DEBUG')) 
 ...
 * Callback by PayPal
 */
elseif ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['payment_type']) && isset($_POST['custom'])) {
    require './auth.php';

    if (defined('PAYPAL_STANDARD_DEBUG')) 
        func_pp_debug_log('paypal_standard', 'C', print_r($_GET, true) . print_r($_POST, true));
    ...
    if ($config['paypal_solution'] != 'uk') {
        ...
        if (defined('PAYPAL_STANDARD_DEBUG')) 
            func_pp_debug_log('paypal_standard', 'validate', print_r($post, true) . print_r($result, true));
