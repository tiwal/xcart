 ...
 * @version    $Id: payment_ccmid.php,v 1.48.2.6 2012/03/27 11:18:43 aim Exp $
...
if (empty($orderids)) {
    ...
    if (!empty($bill_output['hide_mess'])) {
...
if (!$fatal) {
    ...
    if (!empty($bill_output['avsmes'])) {
        $advinfo[] = "AVS info: " . $bill_output['avsmes'];
    }

    if (!empty($bill_output['cvvmes'])) {
        $advinfo[] = "CVV info: " . $bill_output['cvvmes'];
    }

    if (!empty($bill_output['cavvmes'])) {
