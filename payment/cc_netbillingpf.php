 ...
 * @version    $Id: cc_netbillingpf.php,v 1.19.2.5.2.1 2012/04/18 08:40:24 aim Exp $
...
if ($REQUEST_METHOD == 'POST' && !empty($_POST['Ecom_ConsumerOrderID']) && !empty($_POST['Ecom_Ezic_Response_StatusCode'])) {
    ...
    $tr_data = func_query_first("SELECT sessid, param1 FROM $sql_tbl[cc_pp3_data] WHERE ref='".$ordr."'");
    if (!empty($tr_data)) {
        $bill_output['sessid'] = $tr_data['sessid'];
    ...
    if (!$duplicate)
        db_query("REPLACE INTO $sql_tbl[cc_pp3_data] (ref,sessid, param1) VALUES ('".addslashes($nb_oid)."','".$XCARTSESSID."','".addslashes($md5_hash)."')");
