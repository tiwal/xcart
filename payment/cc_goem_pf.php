...
if ($REQUEST_METHOD == 'GET' && isset($_GET['Status']) && isset($_GET['OrderID'])) {
    ...
    $bill_output['sessid'] = func_query_first_cell("SELECT sessid FROM $sql_tbl[cc_pp3_data] WHERE ref='".addslashes($_GET["OrderID"])."'");
    ...
    if (!$duplicate)
        db_query("REPLACE INTO $sql_tbl[cc_pp3_data] (ref,sessid) VALUES ('".addslashes($goem_oid)."','".$XCARTSESSID."')");
