 ...
 * @version    $Id: cc_dibs.php,v 1.15.2.5.2.1 2012/04/18 08:40:23 aim Exp $
...
elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['orderid'])) {
    ...
    if (defined('DIBS_DEBUG')) {
        func_pp_debug_log('dibs', 'I', $post);
    }

    if (!$duplicate)
        db_query("REPLACE INTO $sql_tbl[cc_pp3_data] (ref,sessid,trstat) VALUES ('".addslashes($order)."','".$XCARTSESSID."','GO|".implode('|',$secure_oid)."')");
