 ...
 * @version    $Id: cc_chrono.php,v 1.17.2.6.2.1 2012/04/18 08:40:23 aim Exp $
...
if ($REQUEST_METHOD == 'POST' && !empty($_POST['transaction_id']) && !empty($_POST['cs1'])) {
    ...
    $tmp = func_query_first("select sessid, trstat from $sql_tbl[cc_pp3_data] where ref='".$skey."'");
    ...
    if (!$duplicate)
        db_query("REPLACE INTO $sql_tbl[cc_pp3_data] (ref, sessid, param1, trstat) VALUES ('".$_orderids."', '".$XCARTSESSID."', '".$cart["total_cost"]."', 'GO|".implode('|',$secure_oid)."')");
