 ...
 * @version    $Id: cc_ogoneweb_result.php,v 1.28.2.5.2.1 2012/04/18 08:40:24 aim Exp $
...
if (isset($amount)) {
    ...
    if (isset($currency)) {
        ...
        $payment_return['_currency'] = func_query_first_cell("SELECT param04 FROM $sql_tbl[ccprocessors] WHERE processor='cc_ogoneweb.php'");
    }
}
...
/**
 * This is server2server callback
 */
$module_params = func_query_first("SELECT * FROM $sql_tbl[ccprocessors] WHERE processor = 'cc_ogoneweb.php'");
/**
...
#Level 2 Security Checking
if (!empty($COMPLUS) && $COMPLUS != md5($pp_curr . $pp_merch . $pp_secret . $bill_output['sessid']) && !$is_fake_callback)
    $is_fake_callback = true;
...
if (!$is_fake_callback) {
    ...
    die('Security tests were not passed');
}
?>
