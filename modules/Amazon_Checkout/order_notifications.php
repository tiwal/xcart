 ...
 * @version    $Id: order_notifications.php,v 1.9.2.15 2012/03/27 11:18:27 aim Exp $
...
if ($type == 'NEWORDERNOTIFICATION') {
    ...
    if (func_query_first_cell("SELECT COUNT(amazon_oid) FROM $sql_tbl[amazon_orders] WHERE amazon_oid='$amazon_oid'")) {
        // Already placed!
        func_acheckout_debug("\t+ [Error]: Duplicate NEWORDERNOTIFICATION transaction amazon_oid: #$amazon_oid.");
    ...
    // As orderid is PRIMARY key then use fake (negative) orderid to lock amazon_orders for this $amazon_oid
    $fake_orderid = intval(func_query_first_cell("SELECT MIN(orderid) FROM $sql_tbl[amazon_orders]"));
    $fake_orderid = ($fake_orderid <= 0) ? $fake_orderid-1 : -1;
    @func_array2insert(
        'amazon_orders',
        array(
    ...
    func_acheckout_debug("\t+ amazon_oid #" . $amazon_oid . " is locked with fake orderid #" . $fake_orderid);
    ...
    foreach ($amazon_carts as $ref => $_products) {
        ...
        db_query("DELETE FROM $sql_tbl[amazon_orders] WHERE amazon_oid='$amazon_oid' AND orderid<0 ");
        ...
        $cart = array();
        x_session_save();
        define('X_SESSION_FINISHED', true);//Do not call x_session_save from register_shutdown_function('x_session_finish');
