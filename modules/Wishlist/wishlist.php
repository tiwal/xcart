 ...
 * @version    $Id: wishlist.php,v 1.113.2.10.2.1 2012/04/06 13:01:29 aim Exp $
...
if ($mode == 'addgc2wl') {
    ...
    if (
        ...
        foreach($ids_redirect as $k => $id) {
            $ids_redirect[$k] = func_query_first_cell("SELECT product FROM $sql_tbl[products_lng_current] WHERE productid = '$id'");
        }
    ...
    if ($mode == 'send_friend') {
        ...
        $_userinfo = func_userinfo($logged_userid, $user_account['usertype'], false, false, array('C','H'));
        $mail_smarty->assign('wlid', $logged_userid);
        $mail_smarty->assign('wl_products', $wl_products);
        $mail_smarty->assign('userinfo', func_array_map('func_html_entity_decode',$_userinfo));
    ...
    $mail_smarty->assign('userinfo', $_userinfo);
