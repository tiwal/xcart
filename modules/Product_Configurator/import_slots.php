 ...
 * @version    $Id: import_slots.php,v 1.32.2.7.2.1 2012/04/06 13:01:29 aim Exp $
...
if ($import_step == 'process_row') {
    ...
    if (is_null($_stepid) && empty($import_file['drop']['product_configurator_steps'])) {
    ...
    while (($id = func_export_get_row($data)) !== false) {
        ...
        foreach ($mrow as $row) {
            ...
            if (!empty($row['default_productid'])) {
                $default_product = func_query_first("SELECT p.productcode as default_productcode, pl.product as default_product FROM $sql_tbl[products] AS p INNER JOIN $sql_tbl[products_lng_current] AS pl USING (productid) WHERE p.productid='$row[default_productid]'");
