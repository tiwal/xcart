...
/**
 * This file is included from func_process_order and func_complete_order (include/func/func.order.php)
 *
 * @category   X-Cart
 * @package    X-Cart
 * @subpackage Modules
 ...
 * @version    $Id: capture_order.php,v 1.1.2.1 2012/04/06 10:34:25 ferz Exp $
...
if (func_taxcloud_is_module_enabled() && !isset($order['extra']['taxcloud_returned'])) {
    ...
    if (!isset($order['extra']['taxcloud_authorized'])) {
        $isCaptured = func_taxcloud_authorized_with_capture(
            $userinfo['userid'],
    ...
    } elseif (!isset($order['extra']['taxcloud_captured'])) {

        $isCaptured = func_taxcloud_captured(
            $order['orderid'],
            $error
        );
