 ...
 * @version    $Id: func.php,v 1.24.2.7 2012/03/27 11:18:30 aim Exp $
...
function func_fc_build_categories($tick = 0, $membershipids = false, $languages = false, $display_header = true)
{
    ...
    foreach ($languages as $shop_language) {
        ...
        foreach ($membershipids as $mid) {
            ...
            $cache_path = $cache_pre_path . $user_account['membershipid'] . "." . $shop_language . "." . intval($HTTPS) . ".php";
...
{
    global $shop_language, $user_account, $var_dirs, $fcat_module_path, $config, $HTTPS;

    return $var_dirs['cache'] . '/fc.' . $config['Flyout_Menus']['fancy_categories_skin'] 
        . '.' . intval($user_account['membershipid']) . '.' . $shop_language . '.' . intval($HTTPS) . '.php';
}
