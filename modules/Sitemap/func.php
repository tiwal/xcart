 ...
 * @package    Modules
 * @subpackage Sitemap
 * @version    $Id: func.php,v 1.8.2.11.2.1 2012/04/06 13:01:29 aim Exp $
 * @since      4.4.0
 */
...
function sitemap_get_products_query($count = false, $code = null, $start = null, $end = null, $for = null, $id = null)
{
    ...
    if ($config['Sitemap']['sitemap_multilang'] == 'Y') {
        ...
        $l_join[]   = "{$sql_tbl['products_lng_'.$code]} ON {$sql_tbl['products_lng_'.$code]}.productid = $sql_tbl[products].productid";
    } else {
        $select[] = "$sql_tbl[products_lng_current].product AS name";
        $l_join[]   = "$sql_tbl[products_lng_current] ON $sql_tbl[products_lng_current].productid=$sql_tbl[products].productid";
    }
    ...
    if (
        $config['General']['unlimited_products'] == 'N' // 'Disable inventory tracking' is disabled 
        && $config['General']['show_outofstock_products'] != 'Y' // 'Show products which are out of stock' is disabled
    ) {
        $l_join[] = "$sql_tbl[variants] ON $sql_tbl[variants].productid=$sql_tbl[products].productid";
        $where[] = "IFNULL($sql_tbl[variants].avail, $sql_tbl[products].avail)>0";
        $order_by[] = "$sql_tbl[variants].avail";
    ...
    return sitemap_build_query($select, $from, $where, $order_by, $l_join, $limit, $group_by);
}
 ...
 */
function sitemap_build_query($select, $from, $where, $order_by = null, $l_join = null, $limit = null, $group_by = null)
{
    if (is_array($select)) {
