 ...
 * @version    $Id: import_values.php,v 1.26.2.6 2012/03/27 11:18:30 aim Exp $
 ...
 */
    ...
    foreach ($values['option'] as $k => $v) {
        ...
        if (is_null($_optionid) && empty($import_file['drop']['product_classes'])) {
 ...
 */
    ...
    // Import data...
        ...
        foreach ($row['values'] as $v) {
            ...
            if ($type == 'S' || $type == 'M') {
                $_v = func_query_first_cell("SELECT $sql_tbl[feature_variants].fvariantid FROM $sql_tbl[feature_variants] INNER JOIN {$sql_tbl['feature_variants_lng_' . $config['default_admin_language']]} USING(fvariantid) WHERE $sql_tbl[feature_variants].foptionid='$v[optionid]'  AND variant_name='$v[value]'");
                if (empty($_v))
                    continue;
                ...
                if ($type == 'M') {
                    ...
                    if (is_array($_vv) && !empty($_vv)) {
                        $__vv = func_query_column("SELECT variant_name FROM {$sql_tbl['feature_variants_lng_' . $config['default_admin_language']]} WHERE fvariantid IN ('".@implode("','", $_vv)."')");
                        if (!in_array($_v, $__vv)) {
                            array_push($_vv, $_v);
                        }
                        $_v = $_vv;
        ...
        foreach ($row as $srow) {
            ...
            if (in_array($srow['option_type'], array('S', 'M'))) {
                ...
                if ($srow['option_type'] == "S") {
                    $srow['value'] = func_query_first_cell("SELECT variant_name FROM {$sql_tbl['feature_variants_lng_' . $config['default_admin_language']]} WHERE fvariantid='$srow[value]'");
                ...
                // If feature option - multiple selector
                    ...
                    foreach ($_value as $vk) {
                        ...
                        $p_row['value'][] = func_query_first_cell("SELECT variant_name FROM {$sql_tbl['feature_variants_lng_' . $config['default_admin_language']]} WHERE fvariantid='$vk'");
                    }
                    continue;
                }
            }
