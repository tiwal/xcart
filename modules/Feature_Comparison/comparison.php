...
/**
 * $Id: comparison.php,v 1.53.2.8.2.1 2012/04/06 13:01:29 aim Exp $
 */
...
if ($classes_count > 1) {
    $products = func_query("
    SELECT $sql_tbl[feature_classes].fclassid, IF($sql_tbl[feature_classes_lng].class IS NOT NULL,
           $sql_tbl[feature_classes_lng].class, $sql_tbl[feature_classes].class) as class, $sql_tbl[products].*,
           $sql_tbl[products_lng_current].*
      FROM $sql_tbl[product_features]
     ...
     INNER JOIN $sql_tbl[feature_classes]
      LEFT JOIN $sql_tbl[feature_classes_lng]
        ON $sql_tbl[feature_classes_lng].fclassid   = $sql_tbl[feature_classes].fclassid
       AND $sql_tbl[feature_classes_lng].code       = '$shop_language'
     WHERE $sql_tbl[feature_classes].avail          = 'Y'
       AND $sql_tbl[feature_classes].fclassid       = $sql_tbl[product_features].fclassid
       AND $sql_tbl[product_features].productid     = $sql_tbl[products].productid
       AND $sql_tbl[products].productid IN ('".implode("','", array_keys($productids))."')
     ORDER BY $sql_tbl[feature_classes].fclassid, product");
    if (!empty($products)) {
        $classes = array();
        ...
        foreach ($matrix as $k => $v) {
            ...
            foreach ($v['features'] as $kf => $vf) {
                if (($vf['option_type'] == 'S' || $vf['option_type'] == 'M') && !empty($vf['value'])) {
                    ...
                    $v['features'][$kf]['variants'] = func_query("SELECT $sql_tbl[feature_variants].fvariantid, variant_name as name FROM $sql_tbl[feature_variants] INNER JOIN $sql_tbl[feature_variants_lng_current] USING(fvariantid) WHERE foptionid='$vf[foptionid]' AND $sql_tbl[feature_variants].fvariantid IN ('" . $_product_value . "') ORDER BY orderby");
                }
