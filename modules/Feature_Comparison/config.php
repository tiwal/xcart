 ...
 * @version    $Id: config.php,v 1.44.2.7 2012/03/27 11:18:30 aim Exp $
...
/**
 * Search feature_variants_lng_.. tables
 */
$_lngs = func_query_column("SHOW TABLES LIKE '${xcart_tbl_prefix}feature_variants_lng_%'") ;
foreach ($_lngs as $f => $_lng_table) {
    $tbl_alias = str_replace($xcart_tbl_prefix, '', $_lng_table);
    $sql_tbl[$tbl_alias] = $_lng_table;
}

if (!empty($sql_tbl['feature_variants_lng_en'])) {
    func_set_lng_current('feature_variants_lng_','en');
} else {
    $_code = str_replace('feature_variants_lng_', '', $tbl_alias);
    func_set_lng_current('feature_variants_lng_', $_code);
}
