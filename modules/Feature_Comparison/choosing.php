 ...
 * @version    $Id: choosing.php,v 1.50.2.5 2012/03/27 11:18:30 aim Exp $
...
if (!empty($store_choosing['fclassid']) && $mode == 'choose') {
    ...
    if (!empty($store_choosing['fclassid'])) {
        ...
        foreach ($options as $k => $v) {
            ...
            $options[$k]['variants'] = func_query("SELECT $sql_tbl[feature_variants].fvariantid, $sql_tbl[feature_variants_lng_current].variant_name FROM $sql_tbl[feature_variants] INNER JOIN $sql_tbl[feature_variants_lng_current] USING (fvariantid) WHERE foptionid='$v[foptionid]' ORDER BY orderby");
