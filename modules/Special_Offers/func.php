 ...
 * @version    $Id: func.php,v 1.115.2.24.2.1 2012/04/06 13:01:29 aim Exp $
...
function func_prepare_bonuses_n_conditions_params($params)
{
    ...
    foreach ($params as $k => $param) {
        ...
        switch ($param['param_type']) {
            case 'P':
                $query = "$sql_tbl[products].productcode, $sql_tbl[products_lng_current].product FROM $sql_tbl[products] INNER JOIN $sql_tbl[products_lng_current] USING(productid) WHERE $sql_tbl[products].productid = '".$param["param_id"]."'";
                break;
            case 'C':
...
{
    global $sql_tbl, $fake_product_set_id, $single_mode;
    ...
    if (is_array($result)) {
        ...
        foreach ($result as $key => $bonus) {
            ...
            if ($bonus['bonus_type'] == 'D') {
                ...
                $provider_condition = ($single_mode || empty($provider) ? '1' : "$sql_tbl[products].provider = '".$provider."'");

                $excl_product_set = func_query("SELECT $sql_tbl[products].productid, $sql_tbl[products].productcode FROM $sql_tbl[products] LEFT JOIN $sql_tbl[offer_product_params] ON $sql_tbl[offer_product_params].productid = $sql_tbl[products].productid WHERE $provider_condition AND $sql_tbl[offer_product_params].sp_discount_avail != 'Y'");
...
function func_offer_get_subtotal($products)
{
    ...
    foreach ($products as $product) {
        ...
        if (isset($product['saved_original_price']) && !empty($product['saved_original_price'])) {
            $result += $product['amount']*$product['saved_original_price'];
        } else {
...
function func_offer_set_free_products(&$offers, &$products, &$order_bonuses, $in_cart = true)
{
    ...
    foreach ($products as $pk=>$product) {
        ...
        if ($product['product_type'] == 'C' || isset($product['hidden'])) continue;
