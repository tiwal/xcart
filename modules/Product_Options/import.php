 ...
 * @version    $Id: import.php,v 1.35.2.6 2012/03/27 11:18:35 aim Exp $
 ...
 */
    ...
    if (!empty($import_file['drop']['product_options'])) {
    ...
    // Import pricing data...
        ...
        if (!empty($class['option']) && !in_array($class['type'], array('T','A'))) {
...
// Export data
    ...
    while (($id = func_export_get_row($data)) !== false) {
        ...
        while ($row = db_fetch_array($mrow)) {
            ...
            // Export options
            if (!in_array($row['type'], array('T','A'))) {
