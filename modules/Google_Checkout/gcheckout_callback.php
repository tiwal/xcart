 ...
 * @version    $Id: gcheckout_callback.php,v 1.34.2.22.2.1 2012/04/18 08:40:23 aim Exp $
                ...
                for ($j = 0; $j < count($shipping_requested); $j++) {
                    if (!empty($_allowed_shipping_methods)) {
                        foreach ($_allowed_shipping_methods as $_ship_method) {
                            if (trim(func_insert_trademark($_ship_method['shipping'], 'use_alt')) == utf8_decode($shipping_requested[$j]['shipping'])) {
        ...
        foreach ($user_tmp['addresses'] as $_address_id => $_address) {
            ...
            if (!empty($_address['shipping_methods']) && is_array($_address['shipping_methods'])) {
                foreach ($_address['shipping_methods'] as $_ship_method) {
                    $_ship_method['shipping'] = trim(func_google_encode(func_insert_trademark(utf8_decode($_ship_method['shipping']), 'use_alt')));
...
##############################################################################
    ...
    $_pp3_data = func_query_first("SELECT sessid, trstat FROM $sql_tbl[cc_pp3_data] WHERE ref = '$skey'");
    if ($financial_os != 'REVIEWING' || !preg_match("/^GO|/", $_pp3_data['trstat'])) {
        ...
        func_array2update(
            ...
            "ref = '$skey' AND sessid='$XCARTSESSID'"
        );
