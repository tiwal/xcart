 ...
 * @version    $Id: event_modify.php,v 1.67.2.7.2.2 2012/04/20 06:59:14 aim Exp $
                        ...
                        foreach ($columns as $k => $v) {
                            ...
                            if (func_check_email($email)) {
                                if (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[giftreg_maillist] WHERE recipient_email='".addslashes($email)."' AND event_id='$eventid'") > 0) {
                                    ...
                                    $data = array(
                                        'recipient_name' => addslashes(@htmlspecialchars($v[$recipient_name_index], ENT_QUOTES, $default_charset)),
                                        'status' => 'P'
                                    );
                                    ...
                                    foreach ($v as $fk => $fv) {
                                        $data[$cols[$fk]] = addslashes(@htmlspecialchars($fv, ENT_QUOTES, $default_charset));
                                    }
