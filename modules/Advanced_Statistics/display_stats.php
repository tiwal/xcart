 ...
 * @version    $Id: display_stats.php,v 1.52.2.10.2.1 2012/04/06 13:01:29 aim Exp $
...
if (!empty($current_category)) {
    ...
    $product_viewes = func_query("SELECT p.productid, pl.product, COUNT(ss.id) as views_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[categories] as c INNER JOIN $sql_tbl[products_categories] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid=c.categoryid AND c.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='V' AND $date_condition GROUP BY p.productid ORDER BY views_stats DESC");
    if (!is_array($product_viewes))
        $product_viewes = array();

    // List of product sales
    $product_sales = func_query("SELECT p.productid, pl.product, SUM(ss.multi) as sales_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[categories] as c INNER JOIN $sql_tbl[products_categories] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid=c.categoryid AND c.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='S' AND $date_condition GROUP BY p.productid ORDER BY sales_stats DESC");
    if (!is_array($product_sales))
        $product_sales = array();

    // List of deleted from the cart products
    $product_deleted = func_query("SELECT p.productid, pl.product, COUNT(id) as del_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[categories] as c INNER JOIN $sql_tbl[products_categories] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid=c.categoryid AND c.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='D' AND $date_condition GROUP BY p.productid ORDER BY del_stats DESC");
    if (!is_array($product_deleted))
        $product_deleted = array();
}

$f_product_viewes = func_query("SELECT p.productid, pl.product, COUNT(ss.id) as views_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[featured_products] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='V' AND $date_condition GROUP BY p.productid ORDER BY views_stats DESC");
if (is_array($f_product_viewes))
    $product_viewes = func_array_merge($product_viewes, $f_product_viewes);

$f_product_sales = func_query("SELECT p.productid, pl.product, SUM(ss.multi) as sales_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[featured_products] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='S' AND $date_condition GROUP BY p.productid ORDER BY sales_stats DESC");
...
$f_product_deleted = func_query("SELECT p.productid, pl.product, COUNT(id) as del_stats FROM $sql_tbl[products] as p INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid INNER JOIN $sql_tbl[featured_products] as pc INNER JOIN $sql_tbl[stats_shop] as ss WHERE p.productid=pc.productid AND pc.categoryid = '$current_category[categoryid]' AND ss.id=p.productid AND ss.action='D' AND $date_condition GROUP BY p.productid ORDER BY del_stats DESC");
