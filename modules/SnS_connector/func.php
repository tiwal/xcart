 ...
 * @version    $Id: func.php,v 1.49.2.6.2.1 2012/04/06 13:01:29 aim Exp $
...
function func_sns_select_product($productid)
{
    ...
    $tmp = func_query_first("SELECT $sql_tbl[products_lng_current].product, $sql_tbl[categories].categoryid FROM $sql_tbl[products_lng_current], $sql_tbl[products_categories], $sql_tbl[categories] WHERE $sql_tbl[products_lng_current].productid = '$productid' AND $sql_tbl[products_lng_current].productid = $sql_tbl[products_categories].productid AND $sql_tbl[products_categories].main = 'Y' AND $sql_tbl[products_categories].categoryid = $sql_tbl[categories].categoryid");
    if (empty($tmp))
        return false;
