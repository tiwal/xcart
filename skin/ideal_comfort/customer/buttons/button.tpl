{*
$Id: button.tpl,v 1.1.1.1 2012/04/05 10:16:13 ferz Exp $
...
{else}
  ...
  <button class="button{if $additional_button_class} {$additional_button_class}{/if}" type="{if $type eq 'input'}submit{else}button{/if}"{if !notitle} title="{$title|default:$button_title|escape}"{else} title="{$tips_title|escape}"{/if}{if $js_link} onclick="{$js_link}"{/if}{if $button_id} id="{$button_id|escape}"{/if}>
  {strip}
