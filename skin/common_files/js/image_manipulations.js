 ...
 * @version    $Id: image_manipulations.js,v 1.2.2.3 2011/11/16 11:42:40 aim Exp $
...
function ajax_call_image_manipulations(request) {
  if ($("#fields_image").prop("checked")) {
    request['image_geid'] = geid;
  }
  if ($("#fields_thumbnail").prop("checked")) {
...
function generate_thumbnail_callback(data) {
  ...
  $("#generate_thumbnail").prop("disabled", false);
  $("#image_save_msg").show();
}
...
function gen_thumbnail(id) {
  $("#generate_thumbnail").prop("disabled", true);
...
function delete_image_callback(data, imgid, id, buttonid, type) {
  ...
  if (type == "T" || type == "P") {
    $("#"+type+"image_button3").hide();
    $("#"+type+"image_button3").parent().find("tr>td.SubHeaderLine").parent().hide();
  }  
