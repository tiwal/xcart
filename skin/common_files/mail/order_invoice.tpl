{*
$Id: order_invoice.tpl,v 1.1.2.1 2011/12/15 10:26:02 aim Exp $
...
{/if}
{foreach from=$_userinfo.additional_fields item=v}{if $v.section eq 'P' and $v.value ne ''}
...
{/if}
{foreach from=$_userinfo.additional_fields item=v}{if $v.section eq 'B' and $v.value ne ''}
{$v.title|mail_truncate}{$v.value}
{/if}{/foreach}
...
{/if}
{foreach from=$_userinfo.additional_fields item=v}{if $v.section eq 'S' and $v.value ne ''}
{$v.title|mail_truncate}{$v.value}
{/if}{/foreach}{assign var="is_header" value=""}
{foreach from=$_userinfo.additional_fields item=v}{if $v.section eq 'A' and $v.value ne ''}
{if $is_header ne 'Y'}
