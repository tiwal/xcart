{*
$Id: pconf_search.tpl,v 1.4.2.3 2011/07/12 11:31:03 aim Exp $
...
{if $usertype eq "A"}
{getvar var='providers' func='func_get_providers'}
{if $providers}
<tr>
  ...
  {section name=prov loop=$providers}
    <option value="{$providers[prov].id}" {if $search_data.provider eq $providers[prov].id}selected{/if}>{$providers[prov].login} ({$providers[prov].title} {$providers[prov].lastname} {$providers[prov].firstname})</option>
  {/section}
  </select>
  </td>
</tr>
