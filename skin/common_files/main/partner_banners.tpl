{*
$Id: partner_banners.tpl,v 1.3.2.5 2011/08/29 13:45:13 aim Exp $
...
$(document).ready(function(){
  $('#choosen_partner').change(function() {
    var url = self.location.href;
    url = url.replace(/&current_partner=(\d+)/,'');
    self.location = url + '&current_partner=' + this.value;
  });  
});
{/literal}
...
{elseif $banner_type ne ''}
  ...
  {if $banner.banner_type ne ''}
    ...
    {if $get}
      ...
      {elseif $banner.banner_type eq 'C' and not $categoryid}
      ...
      {else}
        ...
        {capture name=dialog}
          ...
          {if $banner.banner_type eq 'C' or $banner.banner_type eq 'F'}
            <div align="right">
              <a href="partner_banners.php?bannerid={$banner.bannerid}&amp;get=1">{if $banner.banner_type eq 'C'}{$lng.lbl_choose_category}{else}{$lng.lbl_choose_manufacturer}{/if}</a>
            </div>
          {/if}
          ...
          {if $banner.userid eq 0}
            ...
            {if $usertype ne 'B'}
              <br />
              {$lng.lbl_choose_partner}
              {getvar var='partners' func='func_get_partners'}
              <select name="choosen_partner" id="choosen_partner">
                <option value="">PARTNER</option>
                {section name=prov loop=$partners}
                  <option value="{$partners[prov].id}" {if $current_partner eq $partners[prov].id}selected{/if}>{$partners[prov].login} (#{$partners[prov].id}) ({$partners[prov].title} {$partners[prov].lastname} {$partners[prov].firstname})</option>
                {/section}
              </select>
            {/if}
          {else}
