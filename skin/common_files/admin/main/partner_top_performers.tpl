{*
$Id: partner_top_performers.tpl,v 1.1.2.2 2011/08/30 07:02:05 aim Exp $
...
<tr{cycle values=", class='TableSubHead'"}>
  <td>{if $search.report eq 'userid'}<a href="{$catalogs.admin}/user_modify.php?user={$v.userid}&amp;usertype=B">{$v.name|default:$lng.lbl_unknown}</a>{else}{$v.data|escape}{/if}</td>
  <td>{$v.clicks}</td>
  <td>{$v.num_sales}</td>
