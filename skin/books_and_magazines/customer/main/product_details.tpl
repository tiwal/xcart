{*
$Id: product_details.tpl,v 1.1.2.2 2012/04/09 12:43:36 ferz Exp $
...
<form name="orderform" method="post" action="cart.php" onsubmit="javascript: return FormValidation(this);">
  ...
  {/if}
    ...
    {if $product.rating_data && $printable ne "Y"}
  ...
  <table cellspacing="0" class="product-properties" summary="{$lng.lbl_description|escape}">
    ...
    {if $product.weight ne "0.00" or $variants ne ''}
      <tr id="product_weight_box"{if $product.weight eq '0.00'} style="display: none;"{/if}>
        <td class="property-name property-name2">{$lng.lbl_weight}</td>
	...
	<tr>
		...
		{if $product.appearance.has_market_price and $product.appearance.market_price_discount gt 0}
			{$lng.lbl_market_price}: {include file="currency.tpl" value=$product.list_price},
			<span class="you-save">{$lng.lbl_save} {math equation="x-y" x=$product.taxed_price y=$product.list_price assign="you_save"}{include file="currency.tpl" value=$you_save}</span>
		{/if}
		...
		<td valign="top">
        	  <div class="save-percent-container">
	            <div class="save" id="save_percent_box">
        	      {$lng.lbl_save} <span id="save_percent">{$product.appearance.market_price_discount}</span>%
	            </div>
        	  </div>
		</td>
	      {/if}
