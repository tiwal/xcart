 ...
 * @version    $Id: import_orders.php,v 1.30.2.7 2012/03/27 11:18:20 aim Exp $
...
elseif ($import_step == 'export') {
    ...
    while ($id = func_export_get_row($data)) {
        ...
        if ($single_mode || AREA_TYPE == 'A') {
            $row = func_query_first("SELECT $sql_tbl[orders].*, $sql_tbl[customers].login FROM $sql_tbl[orders] LEFT JOIN $sql_tbl[customers] ON $sql_tbl[orders].userid = $sql_tbl[customers].id  WHERE orderid = '$id'");
        } else {
