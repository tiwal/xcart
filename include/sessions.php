 ...
 * @version    $Id: sessions.php,v 1.110.2.11.2.3 2012/04/18 10:32:34 aim Exp $
...
function func_delete_session_related_data($delete_ids)
{
    ...
    db_query("DELETE FROM $sql_tbl[packages_cache] WHERE sessid IN ('" . $deleteSet . '\')');

    // Clear shipping rates cache
    db_query("DELETE FROM $sql_tbl[shipping_cache] WHERE sessid IN ('" . $deleteSet . '\')');

    // Clear user online statistics
    db_query("DELETE FROM $sql_tbl[users_online] WHERE sessid IN ('" . $deleteSet . '\')');

    // Clear cache for TaxCloud module
    if (!empty($sql_tbl['taxcloud_cache']))
        db_query("DELETE FROM $sql_tbl[taxcloud_cache] WHERE sessid IN ('" . $deleteSet . '\')');


    // Clear cache for Amazon_Checkout module
    if (!empty($sql_tbl['amazon_data']))
        db_query("DELETE FROM $sql_tbl[amazon_data] WHERE sessid IN ('" . $deleteSet . '\')');
