 ...
 * @version    $Id: export.php,v 1.74.2.9 2012/03/27 11:18:19 aim Exp $
...
if (is_array($import_specification) && !empty($import_specification)) {
    foreach ($import_specification as $k=>$v) {
        if (!empty($v['export_tpls']) && is_array($v['export_tpls'])) {
...
// Change provider
settype($export_data['provider'], 'string');
...
OUT;
    ...
    foreach ($export_data['check'] as $section) {
        ...
        if (!empty($import_specification[$section]['is_language'])) {
