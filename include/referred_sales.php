 ...
 * @version    $Id: referred_sales.php,v 1.30.2.6.2.1 2012/04/06 13:01:28 aim Exp $
...
if ($REQUEST_METHOD == 'GET' && $mode == 'search') {
    ...
    $sales = array();
    if ($search['top']) {
        $res = db_query("SELECT $sql_tbl[products_lng_current].product, $sql_tbl[products].productid, $sql_tbl[partner_payment].commissions,  $sql_tbl[order_details].amount, $sql_tbl[partner_product_commissions].product_commission, $sql_tbl[order_details].extra_data FROM $sql_tbl[customers] INNER JOIN $sql_tbl[partner_payment] INNER JOIN $sql_tbl[order_details] INNER JOIN $sql_tbl[products] INNER JOIN $sql_tbl[products_lng_current] ON $sql_tbl[products_lng_current].productid=$sql_tbl[products].productid INNER JOIN $sql_tbl[orders] INNER JOIN $sql_tbl[partner_product_commissions] WHERE $sql_tbl[partner_product_commissions].itemid = $sql_tbl[order_details].itemid AND $sql_tbl[partner_product_commissions].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[partner_product_commissions].userid = $sql_tbl[customers].id AND $sql_tbl[customers].usertype = 'B' AND $sql_tbl[customers].status = 'Y' AND $sql_tbl[partner_payment].userid = $sql_tbl[customers].id AND $sql_tbl[partner_payment].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[orders].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[order_details].productid = $sql_tbl[products].productid ".$where_condition);
        ...
        $sales = func_query("SELECT $sql_tbl[partner_product_commissions].product_commission, $sql_tbl[partner_payment].*, $sql_tbl[customers].*, $sql_tbl[order_details].*, $sql_tbl[products_lng_current].product, $sql_tbl[products].productid, $sql_tbl[order_details].extra_data, IF($sql_tbl[partner_payment].paid = 'Y', 'Y', IF($sql_tbl[orders].status IN ('C','P'), 'A', '')) as paid FROM $sql_tbl[customers] INNER JOIN $sql_tbl[partner_payment] INNER JOIN $sql_tbl[order_details] INNER JOIN $sql_tbl[products] INNER JOIN $sql_tbl[products_lng_current] ON $sql_tbl[products_lng_current].productid=$sql_tbl[products].productid INNER JOIN $sql_tbl[orders] INNER JOIN $sql_tbl[partner_product_commissions] WHERE $sql_tbl[partner_product_commissions].itemid = $sql_tbl[order_details].itemid AND $sql_tbl[partner_product_commissions].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[partner_product_commissions].userid = $sql_tbl[customers].id AND $sql_tbl[customers].usertype = 'B' AND $sql_tbl[customers].status = 'Y' AND $sql_tbl[partner_payment].userid = $sql_tbl[customers].id AND $sql_tbl[partner_payment].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[orders].orderid = $sql_tbl[order_details].orderid AND $sql_tbl[order_details].productid = $sql_tbl[products].productid ".$where_condition." GROUP BY $sql_tbl[partner_payment].payment_id, $sql_tbl[order_details].itemid");
