 ...
 * @version    $Id: banner_stats.php,v 1.36.2.7.2.1 2012/04/06 13:01:28 aim Exp $
    ...
    if (!empty($views) || !empty($clicks)) {
        ...
        for($k = 0; $k < $len; $k++) {
            ...
            if ($v['target'] && $v['targetid']) {
                switch ($v['target']) {
                    case 'P':
                        $v['target_name'] = func_query_first_cell("SELECT product FROM $sql_tbl[products_lng_current] WHERE productid = '$v[targetid]'");
                        break;
