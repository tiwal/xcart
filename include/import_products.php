 ...
 * @version    $Id: import_products.php,v 1.74.2.16.2.1 2012/04/06 13:01:28 aim Exp $
 ...
 */
    ...
    if (($empty_categories_list || !empty($import_file['drop']['categories'])) && (empty($values['categoryid']) || empty($values['category']))) {
 ...
 */
    ...
    if (!empty($import_file['drop']['products'])) {
    ...
    $products_cols = func_query_column("SHOW COLUMNS FROM ".$sql_tbl['products']);

    $include_cols = array('product', 'descr', 'fulldescr', 'keywords');
    ...
    // Import products data...
        ...
        if (isset($product['productid'])) {
            ...
            if (!empty($product['product']))
                $_productid = func_query_first_cell("SELECT productid FROM {$sql_tbl['products_lng_'.$config['default_admin_language']]} WHERE product = '".addslashes($product['product'])."' ".$provider_condition);
        }
        ...
        $cols = func_array_merge($products_cols, $include_cols);
        ...
        if (!empty($data)) {
            ...
            func_unset($data, 'product', 'descr', 'fulldescr', 'keywords');
            if ($is_new) {
                ...
                if (!empty($int_descr_data))
                    func_array2update('products_lng_' . $config['default_admin_language'], $int_descr_data, "productid = '$_productid'");
            }
        }
        if (empty($_productid))
            continue;
    ...
    if (func_import_cache_has_records('PWU')) {
        ...
        while (list($pid, $tmp) = func_import_read_cache('PWU')) {
            ...
            if (!func_clean_url_resource_has_record('P', $pid)) {
                $pid_data = func_query_first("SELECT pl.product, p.productcode FROM $sql_tbl[products] AS p INNER JOIN {$sql_tbl['products_lng_' . $config['default_admin_language']]} AS pl USING(productid) WHERE p.productid = '".$pid."'");
