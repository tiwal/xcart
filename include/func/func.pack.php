 ...
 * @version    $Id: func.pack.php,v 1.35.2.9 2012/03/27 11:18:24 aim Exp $
...
function func_get_packages($items, $package_limits, $max_number_of_packs=1000000)
{
    ...
    if (!defined('PACKING_DEBUG')) {
        ...
        $packages = func_query_first("SELECT packages FROM $sql_tbl[packages_cache] WHERE sessid='$XCARTSESSID' AND md5_args='$md5_args'");
    ...
    if ($packages === false) {
        ...
        if (!defined('PACKING_DEBUG')) {
            func_array2insert('packages_cache', array('md5_args' => $md5_args, 'sessid' => $XCARTSESSID, 'packages' => addslashes(serialize($packages))), true);
        }    
