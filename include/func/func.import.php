 ...
 * @version    $Id: func.import.php,v 1.25.2.15.2.1 2012/04/06 13:01:29 aim Exp $
...
function func_import_detect_product($values, $use_provider = true)
{
    ...
    if (!empty($values['product']) && (is_null($_productid) || ($action == "do" && empty($_productid)))) {
        ...
        if (is_null($_productid) || ($action == 'do' && empty($_productid))) {
            $values_not_added++;
            $_productid = func_query_first_cell("SELECT productid FROM $sql_tbl[products_lng_current] WHERE product = '".addslashes($values['product'])."'".$provider_condition);
            if (!empty($_productid))
    ...
    if ($values_exists == $values_not_added && !empty($import_file['drop']['products']))
...
function func_export_get_product($productid)
{
    ...
    return func_query_first("SELECT p.productid, p.productcode, pl.product FROM $sql_tbl[products] AS p INNER JOIN $sql_tbl[products_lng_current] AS pl USING(productid) WHERE p.productid = '$productid'".(empty($provider_sql) ? "" : " AND p.provider = '$provider_sql'"));
}
...
function func_import_detect_category($values)
{
    ...
    if ($values_exists == $values_not_added && !empty($import_file['drop']['categories']))
...
function func_import_section_do()
{
    ...
    if (!empty($import_specification[$section]['finalize']))
...
/*
* Function is used to skip current user in import process 
*/
function func_import_is_current_user($values)
{
    global $logged_userid, $login, $login_type;
