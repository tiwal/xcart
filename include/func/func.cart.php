 ...
 * @version    $Id: func.cart.php,v 1.126.2.64.2.4 2012/04/18 08:40:23 aim Exp $
...
function func_get_customer_zones_avail ($user = 0, $provider = 0, $address_type = 'S')
{
    ...
    if (!empty($customer_info)) {
        ...
        foreach(array('county','country','state','county','zipcode','city') as $v)
            settype($customer_info[$address_prefix . $v], 'string');
...
function func_calculate_taxes(&$products, $customer_info, $shipping_cost)
{
    ...
    if (!empty($active_modules['TaxCloud'])) {
        ...
        include $xcart_dir . '/modules/TaxCloud/taxes.php';
...
function func_calculate_single($cart, $products, $user, $login_type, $provider_for = '')
{
            ...
            if (is_array($products)) {
                ...
                foreach ($products as $k => $v) {
                    ...
                    if (
                        ...
                        $_taxes = func_tax_price($v['price']*$v['amount'], $v['productid'], false, $v['discounted_price'], $customer_info['id'], '', true, $v['amount']);
    ...
    if (!empty($active_modules['TaxCloud'])) {
        include $xcart_dir . '/modules/TaxCloud/calculate_result.php';
    }
...
function func_payment_method_surcharge($total, $paymentid)
{
    ...
    foreach ($payment_methods as $payment) {
        if ($payment['paymentid'] == $paymentid) {
            $surcharge = doubleval($payment['surcharge']);
            break;
        }
    }
    ...
    foreach ($scratch_products as $product_data) {
        ...
        if ($products_array) {
            ...
            if (!func_get_allow_active_content($products_array['provider'])) {
                ...
                $products_array['product']     = func_xss_free($products_array['product']);
...
/**
 * Check if the payment is selectable with this shipping
 */
function func_cart_is_single_CODpayment_selectable($paymentid, $shippingid)
{
...
function check_payment_methods($membershipid)
{
    ...
    $is_cart_empty = func_is_cart_empty($cart);
    ...
    $md5_args = md5(serialize(array(
        ...
        $has_esd,
        $is_zero_total_cost,
        $config['Gift_Certificates']['allow_use_gc_for_buying_gc'],
        isset($config['paypal_solution']) ? $config['paypal_solution'] : '',
        isset($config['paypal_dp_use_xpc_processor_' . $config['paypal_solution']]) ? $config['paypal_dp_use_xpc_processor_' . $config['paypal_solution']] : '',
        isset($config['paypal_dp_use_xpc_' . $config['paypal_solution']]) ? $config['paypal_dp_use_xpc_' . $config['paypal_solution']] : '',
        $is_cart_empty,
        $shop_language
    )));
...
function func_generate_min_amount_warning($min_amount_warns, $productindexes, $products)
{
    ...
    foreach ($products as $k => $v) {
        ...
        $product_name = func_query_first_cell("SELECT $sql_tbl[products_lng_current].product FROM $sql_tbl[products_lng_current] WHERE $sql_tbl[products_lng_current].productid = '$v[productid]'");
...
function func_save_customer_cart($user = false, $_cart = false)
{
    ...
    if (!empty($user)) {
        ...
        $key = md5($serialized_cart) . $user;
        if (isset($saved_carts[$key])) {
            $res = $saved_carts[$key];
        } else {
            $res = db_query("UPDATE $sql_tbl[customers] SET cart='".addslashes($serialized_cart)."' WHERE id='$user'");
            $saved_carts[$key] = $res;
        }    
    }
...
function func_generate_n_save_uniqueid($err_lbl='txt_gcheckout_impossible_error', $max_attempts=10)
{
    ...
    while (true) {
        ...
        @db_query("INSERT INTO $sql_tbl[cc_pp3_data] (ref,sessid,trstat) VALUES ('$unique_id','".$XCARTSESSID."','GO|')");
...
function func_payment_product_description($product, $limit=0, $language='')
{
    ...
    $_descr .= " " . strip_tags(func_query_first_cell("SELECT $sql_tbl[products_lng_current].descr FROM $sql_tbl[products_lng_current] WHERE $sql_tbl[products_lng_current].productid='$product[productid]'"));
 ...
 */
function func_cart_lock($by_process, $run_save = '')
{
 ...
 */
function func_cart_unlock($run_save = '')
{
    global $cart_locked_by_process;
