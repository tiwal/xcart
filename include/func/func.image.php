 ...
 * @version    $Id: func.image.php,v 1.85.2.20 2012/03/27 11:18:24 aim Exp $
...
function func_get_image_ext($mime_type)
{
    ...
    if (preg_match("/^image\/(.+)$/Ss", $mime_type, $m))#nolint
        return $m[1];#nolint
...
function func_image_filename_is_unique($file, $type, $imageid=false)
{
    ...
    if (empty($config['available_images'][$type]) || empty($config['setup_images'][$type])) {
        // ERROR: unknown or not available image type
        return false;
    }
...
function func_prepare_image($file_upload_data, $type = 'T', $id = 0)
{
    ...
    if (!is_url($file_path)) {
        ...
        if (is_array($host) && $host['scheme'] === 'https') {
            ...
            if (!$header)
                return false;
            preg_match("/CONTENT-LENGTH: (\d*)/",strtoupper($header), $out);#nolint
            if (intval($out[1]) > 0)#nolint
                $image = $content;
            else
                return false;
        }
...
function func_generate_image($id, $from_type = 'P', $to_type = 'T', $allow_not_resize = true, $temporary = false)
{
    ...
    if ($temporary) {
        ...
        $_ext = ($config['Appearance']['generated_thumbnail_type'] == 'jpeg' ? 'jpg' : $config['Appearance']['generated_thumbnail_type']);
        $file_upload_data[$to_type] = array_merge(
            $new_image,
            array(
                'is_copied' => true,
                'filename'  => basename($new_image['file_path']).".$_ext",
                'source'    => 'L',
                'id'        => $id,
                'type'      => $to_type,
                'date'      => XC_TIME,
...
{
    global $auto_thumb_error, $xcart_dir, $config;
    ...
    $_func = 'func_image' . $thumb_image_type;
    if (function_exists($_func))
        $res = $_func($_image, $new_file);
    ...
    return array(
        ...
        'image_type'    => "image/$thumb_image_type",
    );
}
...
function func_image_cache_build($type, $id = false, $name = false, $options = 0)
{
    ...
    while (($row = db_fetch_array($res))) {
        ...
        if (
            empty($image_filename)
            && is_url($row['image_path'])
        ) {
            $image_filename = func_get_url_image($row['image_path']);
            $is_temp = true;
        }
