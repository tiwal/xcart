 ...
 * @version    $Id: func.product.php,v 1.124.2.24.2.1 2012/04/06 13:01:29 aim Exp $
...
function func_select_product($id, $membershipid, $redirect_if_error = true, $clear_price = false, $always_select = false, $prefered_image_type = 'P')
{
    ...
    if ($current_area == 'C') {
        ...
        $price_condition = " AND $sql_tbl[pricing].membershipid = '0' AND $sql_tbl[pricing].quantity = '1' AND $sql_tbl[pricing].variantid = '0'";
...
function func_generate_sku($provider, $prefix = false, $max = 0)
{
    ...
    $max_v = empty($active_modules['Product_Options'])
        ? 0
        : intval(func_query_first_cell("SELECT MAX(SUBSTRING($sql_tbl[variants].productcode, ".($len+1).")) AS max FROM $sql_tbl[variants] INNER JOIN $sql_tbl[products] USING(productid) WHERE SUBSTRING($sql_tbl[variants].productcode, 1, $len) = '".addslashes($prefix)."' $provider_cond"));
...
function func_sku_is_unique($sku, $provider, $add_condition='')
{
    ...
    return (
        empty($active_modules['Product_Options'])
        || func_query_first_cell("SELECT COUNT($sql_tbl[variants].variantid) FROM $sql_tbl[variants] INNER JOIN $sql_tbl[products] USING(productid) WHERE $sql_tbl[variants].productcode = '$sku' $provider_cond") == 0
    );
}
