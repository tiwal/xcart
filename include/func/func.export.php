 ...
 * @version    $Id: func.export.php,v 1.28.2.14.2.1 2012/04/06 13:01:28 aim Exp $
...
function func_export_define($data, $parent = '')
{
    ...
    $ret = array();
    foreach ($data as $k => $v) {
        settype($v['parent'], 'string');
        settype($v['orderby'], 'int');
...
function func_export_read_data($section)
{
    ...
    if (!empty($current_code)) {
        ...
        if (!empty($import_specification[$section]['table']))
            $import_specification[$section]['table'] = str_replace("{{code}}", $current_code, $import_specification[$section]['table']);
    }

    $parent_range_query = false;
    if (!empty($import_specification[$section]['parent']) && !in_array($type, array("S","C"))) {
        ...
        while (!empty($import_specification[$s]['parent']) && empty($import_specification[$s]['is_range'])) {
            ...
            if (!empty($current_code)) {
                $import_specification[$s]['table'] = str_replace("{{code}}", $current_code, $import_specification[$s]['table']);
            }
