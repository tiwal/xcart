 ...
 * @version    $Id: func.shipping.php,v 1.23.2.16 2012/03/27 11:18:24 aim Exp $
    ...
    $enable_shipping = (
        (
            ...
            && (
                !empty($login)
                || $config['General']['enable_anonymous_checkout'] == 'Y'
                || func_is_shipping_estimator_request($userinfo)
            )
        )
        || $config['General']['apply_default_country'] == 'Y'
    );
...
function func_is_shipping_estimator_request($userinfo)
{
    return !empty($userinfo['is_incomplete']);
}
...
/*
* Choose cheapest rates for duplicated shippingids. Remove duplicates shippingids
*/
function func_shipping_min_rates($rates)
{
    if (
        !is_array($rates)
        || empty($rates)
    ...
    foreach($rates as $rate) {
        ...
        if (!isset($result[$methodid]))
            $result[$methodid] = $rate;

        // Find min rate for each methodid
        if ($result[$methodid]['rate'] > $rate['rate']) {
            $result[$methodid] = $rate;
        }
    }
