 ...
 * @version    $Id: product_clone.php,v 1.67.2.10.2.1 2012/04/06 13:01:28 aim Exp $
...
foreach ($_languages as $_code) {
    $tables_array[] = array('table'=>'products_lng_' . $_code, 'key_field'=>'productid');
}
 ...
 */
if ($productid != "") {
    $product_info = func_query_first("SELECT * FROM $sql_tbl[products] INNER JOIN $sql_tbl[products_lng_current] USING (productid) WHERE productid='$productid'");
}
...
if ($product_info['provider']==$logged_userid || $single_mode || $current_area == 'A') {
    ...
    if ($new_productid) {
        ...
        foreach ($product_info as $k=>$v) {
            if (
                !is_numeric($k) 
                && !in_array($k, array('productid', 'productcode', 'provider', 'add_date', 'views_stats', 'del_stats', 'sales_stats', 'product', 'descr', 'fulldescr', 'keywords'))
            ) {
        ...
        if (db_affected_rows() < 0) {
            ...
            // Add CLONE word for product title 
            db_query("UPDATE $sql_tbl[products_lng_current] SET product=CONCAT(product,' (CLONE)') WHERE productid='$new_productid'");
        }
