 ...
 * @version    $Id: install.php,v 1.162.2.26.2.1 2012/04/16 12:57:12 aim Exp $
...
<style type="text/css">
<?php
    $logo_params = '?step=' . $current . 
    (defined('XCART_INSTALLER') ? '&type=xcart_installer' : '&type=module_installer');
?>
<!--
.background {
...
} elseif (!func_is_compatible_version() && defined('XCART_INSTALL')) {
...
<div id="footer">
    ...
    <div class="contact_for_help"><?php echo_lng('contact_for_help_link'); ?></div>
</div>
...
<a href="#" class="close-link" onclick="javascript: document.getElementById('dialog-message').style.display = 'none'; return false;"><img src="skin/common_files/images/spacer.gif" alt="Close" class="close-img" /></a>
    ...
    <div align='right'>
   <input type="button" value="<?php echo_lng('contact_for_help_text'); ?>" />
    </div>
    </div>
</div>
<?php
...
function status($var, $error_type='critical')
{
    if ($var === 'warning')
    ...
    $font_error_color = ($error_type == 'critical') ? 'red' : 'blue';
    return $var ? "<font color=\"green\">[".lng_get('status_ok')."]</font>" : "<font color=\"$font_error_color\">[".lng_get('status_failed')."]</font>";
}

function echo_lng_in_error($var, $lng) {
    if (
        !is_string($var)
        && $var !== true
    ) {
        echo '<br /><font color="blue">' .lng_get($lng). '</font>';
    } else {
        echo '';
    }
}
    ...
    foreach($files as $_k=>$file) {
        if (is_dir($templates_repository.XC_DS.$file)) {
            ...
            if (file_exists($templates_directory.XC_DS.$file) && func_md5_file($templates_directory.XC_DS.$file) != func_md5_file($templates_repository.XC_DS.$file)) {
                $ex_files[] = $templates_directory.XC_DS.$file;
            }
        }
        echo '.'; func_flush();
    }
...
function func_generate_secure_key()
{
    ...
    func_setcookie_raw('salt', $salt, 0, '/', func_get_hostname(), false);
...
function func_is_compatible_version()
{
    return 
        defined('COMPATIBLE_VERSION') 
            && (
                constant('COMPATIBLE_VERSION') == '4.5.0'
                || constant('COMPATIBLE_VERSION') == '4.5.x'
            );
}
