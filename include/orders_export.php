 ...
 * @version    $Id: orders_export.php,v 1.68.2.8.2.1 2012/04/06 13:01:28 aim Exp $
...
while ($data = db_fetch_array($orders_result)) {
    ...
    if (!empty($data['itemid'])) {
        ...
        $order_details = db_query("SELECT p.*, od.*, pl.* FROM $sql_tbl[order_details] AS od LEFT JOIN $sql_tbl[products] AS p ON od.productid = p.productid INNER JOIN $sql_tbl[products_lng_current] AS pl ON pl.productid=p.productid WHERE od.orderid = '$row[orderid]'");
        if ($order_details) {
