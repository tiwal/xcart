 ...
 * @version    $Id: process_product.php,v 1.69.2.8.2.1 2012/04/06 13:01:28 aim Exp $
...
if ($mode == 'delete' && $REQUEST_METHOD == 'GET') {
    ...
    if (is_array($products_to_delete['products'])) {
        ...
        $products = func_query("SELECT $sql_tbl[products].productid, $sql_tbl[products].productcode, $sql_tbl[products_lng_current].product, $sql_tbl[products].provider FROM $sql_tbl[products] INNER JOIN $sql_tbl[products_lng_current] USING(productid) WHERE $search_condition ORDER BY product, productcode");
