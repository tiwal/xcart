 ...
 * @version    $Id: cart.php,v 1.211.2.20 2012/03/27 11:18:13 aim Exp $
 * @link       http://www.x-cart.com/
 * @see        ____file_see____
 */

//$_COOKIE["xid_793fc"]=null;
...
if (
    ...
    if (
        ...
        if (empty($shippingid)) {
            // Reset active COD payment method if COD shipping is changed to nonCOD shipping
            $is_cod_disabled_payment = 
                func_query_first_cell("SELECT is_cod FROM $sql_tbl[shipping] WHERE shippingid='$cart[shippingid]'") == 'Y' &&
                func_query_first_cell("SELECT is_cod FROM $sql_tbl[payment_methods] WHERE paymentid='$cart[paymentid]'") == 'Y';
            
            if ($is_cod_disabled_payment) {
